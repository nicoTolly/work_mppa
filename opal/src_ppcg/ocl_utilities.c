...
cl_device_id opencl_create_device(int use_gpu)
{
	...
	if (err == CL_DEVICE_NOT_FOUND)
		err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_CPU | CL_DEVICE_TYPE_ACCELERATOR, 1, &dev,
				NULL);
	if (err < 0) {
...
cl_device_id opencl_create_device_by_type(cl_device_type device_type)
{
	...
	// Query some OpenCL platforms (2)
	err = clGetPlatformIDs(2, platforms, &nb_platforms);
	if (err < 0) {
		fprintf(stderr, "Error %s while looking for a platform.\n",
				opencl_error_string(err));
		exit(1);
	...
	// Loop over platforms to find a matching device device
	for(i = 0; i < nb_platforms; i++){
		...
		switch(err){
			case CL_SUCCESS          : return dev;
			case CL_DEVICE_NOT_FOUND : break;
			default : {
				fprintf(stderr, "Error %s while getting device on platform %d.\n",
						opencl_error_string(err), i);
				exit(1);
			}
		}
	}
	...
	const char *opencl_options)
{
	...
	// printf("\n\nProgram_size = %d - Program source is : \n %s\n", program_size, 
	// 	program_source);
